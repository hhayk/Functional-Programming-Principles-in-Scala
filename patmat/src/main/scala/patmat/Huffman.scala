...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    (scala.collection.immutable.SortedMap.empty[Char, Int] ++ chars.groupBy(identity).mapValues(_.size)).toList
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    freqs.map { case (c, i) => Leaf(c, i) }.sortBy(l => l.char)
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: tail => (makeCodeTree(left, right) :: tail).sortWith((t1, t2) => weight(t1) < weight(t2))
    case _ => trees
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def rec(ct: CodeTree, bits: List[Bit]): List[Char] = ct match {
      case Leaf(c, _) if bits.isEmpty => List(c)
      case Leaf(c, _) => c :: rec(tree, bits)
      case Fork(left, right, _, _) if bits.head == 0 => rec(left, bits.tail)
      case Fork(left, right, _, _) => rec(right, bits.tail)
    }
